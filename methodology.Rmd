---
title: "Application Design and Exploratory Data Analysis"
toc: true
---
...
comb$time <- format(as.POSIXct(comb$datetime,format="%d-%m-%Y %H:%M:%S"),
                    "%H:%M:%S")
...
```{r}
comb <- as_tibble(comb) %>% mutate_if(is.character, function(col) 
  iconv(col, to="UTF-8"))
```
...
g2 <-
  ...
  ggtitle("Network Graph of Mentions using Centrality") +
...
```{r}
news_group <- c("@AbilaPost", 
                "@KronosStar",
                "@CentralBulletin", 
                "@InternationalNews", 
                "@NewsOnlineToday", 
                "@HomelandIlluminations")

news <- comb1_mentions2 %>% 
  dplyr::filter(mentions %in% news_group) %>% 
  dplyr::select(Text, time, mentions) %>% 
  arrange(time) %>% 
  distinct(Text, time) 
...
Shifting the filter period earlier, there was a hashtag #jerkdrivers about a black van hitting a car, then a bicycle before driving into  parking lot. Since Events C and D describes the sequence of events of the black van and I will group them as one event called Police standoff.
...
For example, we see that the hot and run was very short but generated a lot of tweets as compared to the stand-off. The heatmap was also useful in visualising the speakers identified by the workpair network and shows the time they took the stage at the POK rally. 
...
Three major events have been identified: the POK Rally (Event A), Fire (Event B) and Police Stand-off (Events C and D). As the POK Rally ended peacefully, I will focus on the Fire and Police Stand-off when assessing the risk and safety of the affected residents during these events.  

Some keys authors have been earlier identfied to be involved in the Fire and Police Stand-off. Filtering for their tweets will allow us to get a better assessment of what is happening on the ground.

![](Img/aut1.jpg) 


*GreyCatCollectibles* was identified earier as a possibly the first one who tweeted about the fire. Filtering for this author shows that *dancing dolphin* had been undergoing some renovations for some weeks. *GreyCatCollectibles* was also the one who called 555 at 18:38, and at 18:40 there was a call centre report of "Possible fire". Her tweets details how the fire has not been contained after 19:10 as she tweeted that, "My doll collection.. ... #allruined" indicating that the fire has consumed them. She was also within sight of the building at this time. 
Another indiction that the risk level has increase is when *ccdata* and *HomelandIllumninations* reported that extra fire units were dispatched and the residents were further evacuated from the burning building at around 19:25. Also later at 20:35 where *GreyCatCollectibles* tweeted,"oh my oh my oh my. Everything is gone!" indicating this might be the time where the top floor collapsed.  

```{r}
comb1_authors2 %>% filter(author == "GreyCatCollectibles") %>% 
  dplyr::select(Text, time)
```


![](Img/stand.jpg)  
Authors previously identified to be in the Police Standoff are *megaMan*, *truccotrucco* and *sofitees*. *megaMan* was caught in the parking lot during the stand-off and tweets that the police stopped the back van from leaving the parking lot, causing the "black van faces off two cops, shots fired, people undercover, not moving until its under control." at 19:41:07 and later "don't think anyone ran for it, they didn't have time before police pulled in" at 19:41:45. These people were caught during the cross fire between the people in the black van and the police and were exposed to high risk of being accidentally shot during the stand-off. 
One policeman was reported to have been shot then and send by ambulance to the hospital. 
By 19:43, there were no more shooting and *AbilaPost* reported that the police had started to evacuate people from the scene. 
At 20:01, *megaMan* tweet that "I'm just seeing someone talking to van guys" signalling the start of the negotiations.  
At about the same time, he also tweeted that there were "groups of people running out of stores" and "People trying to leave in their cars - polic says no." adding to the chaos of the moment and raising the risk level as the people in the black van are still armed. 
The AbilaPost reported "close to 200 people have gathered to watch standoff..." at 20:22:21. 

```{r}

comb1_authors2 %>% filter(author == "megaMan") %>% dplyr::select(Text, time)
```   

![](Img/map.jpg)  

*dangermice* and *hennyhenhendrix* were previously identified from the map. Looking at their tweets, I found out that *dangermice* is very near to location of *Dancing Dolphin* as he is able to see the fire. While *hennyhenhendrix* car was hit by the black van before it knocked down the bicyclist.  
